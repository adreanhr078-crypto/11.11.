...
function registryValue(key: string, value: string): string | undefined {
  ...
  return line?.split(/\s{2,}/).at(-1)?.trim() || undefined;
}

export function readConfiguredPath(envKey: string): string | undefined {
  return process.env[envKey]?.trim() || registryValue('HKCU\\Environment', envKey);
}
...
function checkExecutable(spec: ExecutableSpec): ToolCheck {
  ...
  const executable = firstExisting([...envCandidates, ...commandCandidates, ...(spec.knownPaths ?? [])]);
  if (!executable) {
    return {
      id: spec.id,
...
function packageCheck(id: string, packageName: string, requirement: ToolRequirement, note: string): ToolCheck {
  const packagePath = join(REPO_ROOT, 'node_modules', ...packageName.split('/'), 'package.json');
...
function ffprobeCandidates(): string[] {
  ...
  return [configured, ffmpeg ? join(dirname(ffmpeg), process.platform === 'win32' ? 'ffprobe.exe' : 'ffprobe') : ''].filter(Boolean);
}
...
export function checkTools(): ToolCheck[] {
  ...
  const executableChecks = [
    checkExecutable({
      ...
      knownPaths: ['C:/Tools/Blender-5.2.1-x64/Blender Foundation/Blender 5.2/blender.exe'],
      versionArgs: ['--version'],
    ...
    checkExecutable({
      ...
      requirement: 'required',
      envKeys: ['EDGE_EXE'],
      commands: [process.platform === 'win32' ? 'msedge.exe' : 'microsoft-edge'],
      knownPaths: [
  ...
  return [
    ...executableChecks,
    packageCheck('gltf-transform', '@gltf-transform/cli', 'required', 'Pinned GLB optimization and Meshopt pipeline.'),
    packageCheck('gltf-validator', 'gltf-validator', 'required', 'Khronos glTF 2.0 structural validation.'),
    packageCheck('sharp', 'sharp', 'required', 'Non-admin image metadata and poster processing.'),
    {
      id: 'unity',
      requirement: 'deferred',
      found: false,
      healthy: false,
      note: 'Deliberately deferred until the browser vertical slice earns a native-runtime greenlight.',
    },
    {
      id: 'comfyui',
      requirement: 'deferred',
...
export function requiredToolFailures(tools: ToolCheck[]): ToolCheck[] {
  return tools.filter((tool) => tool.requirement === 'required' && (!tool.found || !tool.healthy));
}
