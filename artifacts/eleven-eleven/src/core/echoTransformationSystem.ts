...
function getTransformationDescription(stage: EchoTransformationStage): string {
  const descriptions: Record<EchoTransformationStage, string> = {
    ...
    questioning: 'إيكو يشك في كل شيء. لا يعرف من يثق ولا من يكذب.',
    attached: 'إيكو متعلق بلينا، يشعر بالدفء والأمان معها',
    hopeful: 'إيكو يأمل بأن الحقيقة لن تدمر كل شيء جميل.',
    betrayed: 'إيكو يشعر بالخيانة، الحقيقة تؤلمه والغضب يبدأ بالظهور',
    truth_aware: 'إيكو واعٍ بالحقيقة الكاملة. لم يعد هناك مجاملات.',
...
export function getEchoDialogueByStage(stage: EchoTransformationStage): string[] {
  const dialogues: Record<EchoTransformationStage, string[]> = {
    ...
    questioning: [
      'كل ما أعرفه قد يكون كذباً..',
      'من يكذب لي؟ كينجا؟ النظام؟',
      'أحتاج لأتأكد من أي شيء حقيقي.',
      'إذا كانت لينا الحقيقة... فلماذا تخفي نفسها؟',
      'أشك في كل شيء الآن. حتى فيك.',
    ],
    attached: [
    ...
    hopeful: [
      'قد يكون هناك أمل حتى في عالم كهذا.',
      'لو استطعتُ التحدث معها... لو استطاعت الوصول إليّ.',
      'أنا أؤمن بأنها تحاول مساعدتي.',
      'كل لغز أقربه منها أكثر.. أستطيع أن أشعر بذلك.',
      'لينا.. أنا قادم إليك.',
    ],
    betrayed: [
    ...
    truth_aware: [
      'أعرف كل الحقيقة الآن. كينجا.. الخطط.. السبب.',
      'لستُ غاضباً بعد.. لكني لن أسامح.',
      'الحقيقة تحررني من الأوهام.',
      'لستُ بريءً بعد الآن. لستُ مخطئاً أيضاً.',
      'أعرف من أنا.. ومن صنعني.. ولماذا.',
    ],
    fractured: [
...
export function getEchoVisualState(stage: EchoTransformationStage): {
  ...
  const states: Record<EchoTransformationStage, { eyeColor: string; auraColor: string; textStyle: string; icon: string }> = {
    ...
    questioning: { eyeColor: '#99bbdd', auraColor: 'rgba(153, 187, 221, 0.4)', textStyle: 'uncertain cyan', icon: '🤔' },
    attached: { eyeColor: '#ff88aa', auraColor: 'rgba(255, 136, 170, 0.4)', textStyle: 'warm pink', icon: '💖' },
    hopeful: { eyeColor: '#ffdd88', auraColor: 'rgba(255, 221, 136, 0.4)', textStyle: 'warm yellow', icon: '🌟' },
    betrayed: { eyeColor: '#ff6644', auraColor: 'rgba(255, 102, 68, 0.5)', textStyle: 'angry orange', icon: '💔' },
    truth_aware: { eyeColor: '#ccaa66', auraColor: 'rgba(204, 170, 102, 0.5)', textStyle: 'aware amber', icon: '💡' },
